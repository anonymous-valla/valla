...
class BaseVaLLA(torch.nn.Module):
    """
    Defines a Variational Linearized Laplace Approximation model.
    ...
    num_data : int
        Amount of data samples in the full dataset. This is used to scale the
        likelihood in the loss function to the size of the minibatch.
    output_dim : int
    ...
    fix_inducing_locations : Boolean
        If True, the inducing locations are fixed to their initial value.
    alpha : float
    ...
    def test_step(self, X, y):
        ...
        float
            The nelbo of the model at the current state for the given inputs.
        torch.Tensor of size (batch_size, output_dim)
            Predictive mean of the model on the given batch
        torch.Tensor of size (batch_size, output_dim, output_dim)
    ...
    def compute_inducing_term(self, Kz):
        """
        Computes the term of the ELBO that only considers the inducing locations.
        
        Parameters
        ----------
        Kz : torch.Tensor of shape (num_inducing, num_inducing)
            Contains jacobian kernel evaluated on the inducing locations
            at their corresponding outputs.
    ...
    def compute_kernels(self, X):
        """
        Computes the forward pass and all kernel matrices for the given input
        ...
        torch.Tensor of shape (batch_size, output_dim)
            Contains the forward pass of the DNN for the given inputs.
        torch.Tensor of shape (batch_size, output_dim, output_dim)
            Contains the diagonal kernel matrix evaluated at the inputs.
        torch.Tensor of shape (batch_size, num_inducing, output_dim)
            Contains the kernel matrix evaluated at the inputs and inducing locations.
        torch.Tensor of shape (num_inducing, num_inducing)
            Contains the kernel matrix evaluated at inducing locations.
        """
    ...
    def predict_f(self, X):
        ...
        torch.Tensor of size (batch_size, output_dim)
            Predictive mean of the model on the given batch
        torch.Tensor of size (batch_size, output_dim, output_dim)
            Contains the covariance matrix of the model for each element on the batch.
        """
...
class VaLLARegression(BaseVaLLA):
    """
    Defines a Variational Linearized Laplace Approximation model for regression tasks.
    ...
    output_dim : int
        Dimensionality of the targets.
    log_variance : float
        Initial value for the logarithm of the noise variance.
    track_inducing_locations : Boolean
        If True, an history of the inducing locations is stored.
    fix_inducing_locations : Boolean
        If True, the inducing locations are fixed to their initial value.
    ...
    def freeze_ll(self):
        """Sets the log variance of the noise as non trainable. """
    ...
    def predict_y(self, X):
        """
        Computes the predicted labels for the given input using the likelihood
...
class VaLLARegressionBackend(VaLLARegression):
    """
    Defines a Variational Linearized Laplace Approximation model for regression tasks
    using BackPack interface to compute the Jacobians.
    ...
    log_variance : float
        Initial value for the logarithm of the noise variance.
    backpack : object
        Must implement methods jacobians and jacobians_on_outputs to compute the
        jacobians of the model wrt the inputs and outputs.
    track_inducing_locations : Boolean
        If True, an history of the inducing locations is stored.
    fix_inducing_locations : Boolean
        If True, the inducing locations are fixed to their initial value.
...
class VaLLAMultiClass(BaseVaLLA):
    """ Defines a Variational Linearized Laplace Approximation model for multiclass
    problems. """
...
class VaLLAMultiClassBackend(VaLLAMultiClass):
    """
    Defines a Variational Linearized Laplace Approximation model for multiclass tasks
    using BackPack interface to compute the Jacobians.
